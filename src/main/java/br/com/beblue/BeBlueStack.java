...
/**
 * Como você implementaria uma classe de pilha de inteiros que possui os métodos
 * push, pop e min. Onde min retorna o menor inteiro na pilha e todos os métodos
 * são O(1).
 * 
 * @author falvojr
 */
public class BeBlueStack {
	...
	public void push(Integer newItem) {
		...
		if (this.stack.isEmpty()) {
			...
			// Antes isso, executamos uma equacão para o cálculo do valor de referência (que viabilizará as operacões em O(1)).
    ...
    public Integer peek() {
        ...
        // Caso o item do pop (topo) for menor que o mínimo, então ele é o próprio mínimo. Senão o item é retornado normalmente.
        final Integer topItem = this.stack.peek();
        return topItem < this.min ? this.min : topItem;
    }

    /**
     * Equacao generica para manipular a inclusão de elementos minimos na pilha.
     * Essencial para para que o algoritmo seja O(1).
     */
